...
pub fn Spreadsheet() -> impl IntoView {
    ...
    let handle_keydown = move |event: KeyboardEvent| {
        let mut row = current_row.get();
        let mut col = current_col.get();
        let mut scrolled = false;
        let key = event.key();
        let motion = key.as_str();
        
        let  ( mut offset_row, mut offset_col) = (0,0);
        
        let cell = Arc::clone(&table_data3[row][col]);
        match motion {
            "ArrowUp" => {
        ...
        if 1 <= row && row <= MAX_ROWS && 1 <= col && col <= MAX_COLS {
            let (r, c) = parse_cell_reference(source_cell.get());
            if(r+offset_row>=row+DIM ||r+offset_row<row ||c+offset_row<col ||  c+offset_col>col+DIMB) {
                set_source_cell.set(format!("{}{}", get_column_name(c+offset_col),r+offset_row ));
                let cell = Arc::clone(&table_data3[r-row+offset_row][c-col+offset_col]);
        ...
        // set_current_row.set(offset_row+row);
        // set_current_col.set(offset_col+col);
        
    
        // if new_row-current_row.get() >=DIM || new_col-current_col.get()>=DIM {
        //     handle_cell_edit(
        //         "SCROLL".to_string(),
        //         0,
        //         0,
